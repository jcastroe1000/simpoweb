<?php
    ...
    $cost=$_GET['cost'];
    $places_ava_ws=$_GET['places_available'];
    ...
    if ($_FILES['file_image']['size'] <= 2097152) {
        while (true) {
            ...
            if (!file_exists('../gallery/album/workshop/' . $filename)) break;
        }
    ...
    $query1 = "INSERT INTO registro_eventos(nombre,resumen,dirigido, objetivo,periodo, duracion,dias,"
            ...
            . "'$time_start','$time_finish','$modality','$requiriments','$aditional_information','$cost',$places_ava_ws,'$pay_method','$filename',"
            . "'$creation_date','$section','$created_by')";
        mysqli_query($mysqli, $query1);
        error_log("Despues del primer insert", 0);
        // Movemos el archivo
        move_uploaded_file($_FILES['file_image']['tmp_name'], '../gallery/album/workshop/' . $filename);
