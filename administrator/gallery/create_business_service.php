<?php
    ...
    $section='servicio empresarial';
    $filename = "";
    if ($_FILES['file_image']['size'] <= 2097152) {
        while (true) {
            $filename = uniqid(rand()) . '.' .pathinfo($foto, PATHINFO_EXTENSION);
            if (!file_exists('../gallery/album/business_service/' . $filename)) break;
        }
    ...
    $query1 = "INSERT INTO registro_eventos(nombre,resumen,dirigido, objetivo,periodo, duracion,dias,"
        ...
        // Movemos el archivo
        move_uploaded_file($_FILES['file_image']['tmp_name'], '../gallery/album/business_service/' . $filename);
