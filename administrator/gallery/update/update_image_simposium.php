...
if ($_FILES['file_image']['size'] <= 2097152) {
    ...
    if ($stmt->execute()):
        // Movemos el archivo
        move_uploaded_file($_FILES['file_image']['tmp_name'], '../album/simposium/' . $filename);
        unlink('../album/simposium/'.$id_image_sim);
