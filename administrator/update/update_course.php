<?php
    ...
    $stmt = $mysqli->prepare("UPDATE registro_eventos SET  nombre=? , resumen=?, dirigido=?, periodo=?,
                             duracion=?, dias=?,fecha_inicio=?,fecha_final=?,modalidad=?, requisitos=?,
                             informacion_adicional=?,costo=?,metodo_pago=?,fecha_modificacion=?,seccion=?,
                             usuario=? WHERE id=?");
    $stmt->bind_param('sssssssssssssssss', $course_name,$review,$addressed,$period,$duration,$days_converts,
                            $d_start,$d_finish,$modality,$requiriments,$aditional_information,
